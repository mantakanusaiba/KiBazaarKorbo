...
# New fresh output file
OUT_PATH = os.path.join(BASE_DIR, "data", "processed", "market_prices_all.csv")

# New progress file, separate from previous old scripts
PROGRESS_PATH = os.path.join(BASE_DIR, "data", "processed", "fetch_all_progress.txt")
...
def main():
    ...
    parser.add_argument("--fresh", action="store_true", help="Delete old output/progress and start fresh")
    args = parser.parse_args()

    if args.fresh:
        if os.path.exists(OUT_PATH):
            os.remove(OUT_PATH)
            print(f"Deleted old output: {OUT_PATH}")

        if os.path.exists(PROGRESS_PATH):
            os.remove(PROGRESS_PATH)
            print(f"Deleted old progress: {PROGRESS_PATH}")
    ...
    print(f"Fetching {args.days} days")
    ...
    print("Done.")
    print(f"Fresh dataset saved at: {OUT_PATH}")
