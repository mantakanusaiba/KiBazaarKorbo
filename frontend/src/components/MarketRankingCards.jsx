        ...
        <div>
            ...
            >
                {top5.map((m, i) => (
                    ...
                    >
                        <span style={{ fontSize: 18, width: 26 }}>
                            {MEDALS[i] || `#${bnNum(i + 1)}`}
                        </span>
                        ...
                        <div style={{ flex: 1 }}>
                            ...
                            {!m.covers_all_items && m.covers_n_of && (
                                <div
                                    style={{
                                        fontSize: 11.5,
                                        color: "var(--gray-500)",
                                ...
                                >
                                    {bnNum(m.covers_n_of[1])}টির মধ্যে{" "}
                                    {bnNum(m.covers_n_of[0])}টি পণ্য আছে
                                </div>
                            )}
                        </div>
                        ...
                        <div
                            style={{
                                fontWeight: 700,
                                fontSize: 15,
                                color: i === 0 ? "var(--green-700)" : "var(--gray-900)",
                                whiteSpace: "nowrap",
                            }}
                        >
                            {bnTk(m.total, 2)}
            ...
            {ranking.length > 1 && cheapest.total < ranking[1].total && (
                ...
                >
                    <b>{marketLabel(cheapest.market)}</b> থেকে কিনলে পরের সেরা অপশনের তুলনায় প্রায়{" "}
                    {bnTk(ranking[1].total - cheapest.total, 2)} সাশ্রয় হতে পারে।
                </div>
            )}
        </div>
    );
